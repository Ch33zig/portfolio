...
export const SECTIONS: Section[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        body: [
          {
            text: "The two cofounders I met through [Scripty] had spent years advising early stage startups. They had a course, hours of recorded content, a library of frameworks, and a methodology that had worked for a lot of founders. What they didn't have was a way to scale themselves. There were only two of them, and their students needed guidance at all hours, between sessions, at the exact moment a question came up. That gap was the product.",
            links: { Scripty: "/entry/scripty" },
          },
          "Heydena was a real-time conversational startup advisor. Not a chatbot that answered questions when asked, but an AI that initiated conversations, ran exercises, pushed founders through the material, and remembered everything about their specific startup from one session to the next. The voice behind it was modelled directly on one of the cofounders. That was the whole point: not a generic assistant, but something that felt like having that specific advisor available at any moment.",
          "The architecture had three distinct layers that had to work together. The first was retrieval. The course content was chunked, embedded, and stored in Pinecone, and every conversation pulled relevant material at query time. The second was memory. Each user had a structured profile that tracked their problem statement, ICP, and value proposition, alongside persistent conversation history. Dena wasn't starting from scratch each time. It knew where you were in the course, what you were working on, and what you'd said before. The third was proactivity. Dena didn't wait to be spoken to. It reached out after a user finished a video, checked in after a day away, and re-engaged after longer periods of inactivity. The goal was to behave like an advisor who was actually paying attention, not a tool that sat idle until summoned.",
          ...
          "The only real solution was iteration with actual users. We worked closely with early founders going through the course, collecting feedback not just on whether answers were correct but on whether the conversation felt right. Was it pushing too hard? Not hard enough? Did it feel like an advisor or a search engine? That feedback loop shaped the system prompt more than anything else. Fine-tuning the personality in a conversational AI is really a process of writing, testing, watching real users react, and rewriting. You can't get it right in a vacuum. You only get it right by watching someone use it and noticing the exact moment it stops feeling human.",
          "What I came away understanding is that the technical layer, the retrieval, the memory, the streaming, is the foundation, but it's not what users experience. What users experience is whether the conversation feels like someone is actually there. Building that feeling at scale, reliably, across different users and different moods and different questions, is the real problem. And it is genuinely hard.",
        ],
      },
      {
        id: "synapse",
